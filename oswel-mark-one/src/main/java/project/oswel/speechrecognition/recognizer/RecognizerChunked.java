...
/**
 * This class uses Google's V2 Hook. 
 * The class is returns a chunked respones so listeners must be used.
...
public class RecognizerChunked {
	 ...
	 * Google's API V2 URL
	 */
	private static final String GOOGLE_SPEECH_URL_V2 = 
	 ...
	 * An API key can be obtained by requesting
	...
	/**
	 * The current language the Recognizer is set to use. 
	 * Returns the ISO-Code otherwise,
	 * it may return "auto."
	 ...
	 */
	public void getRecognizedDataForFlac(File infile, int sampleRate) 
	throws IOException{
	 ...
	 * @throws IOException if something goes wrong reading the file.
	 */
	public void getRecognizedDataForFlac(String inFile, int sampleRate) 
	throws IOException{
	 ...
	 * @param data The data you want to send to Google. Speech files 
	 * under 15 seconds long recommended.
	 * @param sampleRate The sample rate for your audio file.
	 */
	private void openHttpsPostConnection(
		final String urlStr, final byte[] data, final int sampleRate) {
		new Thread () {
			public void run() {
				...
				try {
					...
					httpConn.setAllowUserInteraction(
						false);
					...
					if(resCode >= HttpURLConnection.HTTP_UNAUTHORIZED){
						// Stops here if Google doesn't like us/
						// throw new HttpException(
						//	HttpURLConnection.HTTP_UNAUTHORIZED); //Throws
						System.out.println("Unauthorized HTTP request...");
                        System.exit(1);
					}
	...
	private byte[] mapFileIn(File infile) throws IOException{
		...
		try{
			...
			// Ensures resources are closed regardless of whether 
	 ...
	 * @params The string you want to process.
	...
	/**
	 * Adds responseListener that triggers when a response 
	 * from Google is recieved
