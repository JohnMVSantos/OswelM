...
public class Microphone implements Closeable {
	 ...
	 *
	 * @return PROCESSING_AUDIO is returned when the Thread is recording 
	 * 		   Audio and/or saving it to a file<br> STARTING_CAPTURE is 
	 * 		   returned if the Thread is setting variables<br> CLOSED is 
	 * 		   returned if the Thread is not doing anything/not capturing audio
	 */
	 ...
	 *            State from enum
	 */
	private void setState(CaptureState state) { this.state = state; }
	
	public File getAudioFile() { return audioFile; }
	
	public void setAudioFile(File audioFile) { this.audioFile = audioFile; }
	...
	public TargetDataLine getTargetDataLine() { return targetDataLine; }
	...
	private void initTargetDataLine() {
		...
		try {
			setTargetDataLine((TargetDataLine) AudioSystem.getLine(
				dataLineInfo));
	 ...
	 *
	 * @param audioFile
	 *            The fully path (String) to a file you want to 
	 * 			  save the audio in
	 */
	 ...
	 *
	 * @return Returns AudioFormat to be used later when capturing 
	...
	/**
	 * Close the microphone capture, saving all processed 
	 * audio to the specified file.<br>
