...
public class AudioPlayer extends Thread {
	...
	/**
	 * AudioPlayer which can be used if audio stream is to be 
	 * set separately, using setAudio().
	 *
	 */
	public AudioPlayer() {
	}
	 ...
	 */
	public AudioPlayer(
		File audioFile, SourceDataLine line, LineListener lineListener) 
		throws IOException, UnsupportedAudioFileException {
		this.ais = AudioSystem.getAudioInputStream(audioFile);
		this.line = line;
		this.lineListener = lineListener;
	 ...
	 */
	public AudioPlayer(
		AudioInputStream ais, SourceDataLine line, LineListener lineListener) {
		this.ais = ais;
		this.line = line;
		this.lineListener = lineListener;
	}
	...
	public AudioPlayer(
		...
		int outputMode) throws IOException, UnsupportedAudioFileException {
		this.ais = AudioSystem.getAudioInputStream(audioFile);
		this.line = line;
		this.lineListener = lineListener;
	 ...
	 * 			  do the same with the right output channel.
	 */
	public AudioPlayer(
	...
	/**
	 * Sets Gain value. Line should be opened before calling this method. 
	 * Linear scale 0.0 <--> 1.0 Threshold Coef. : 1/2 to avoid saturation.
	 * 
	 * @param fGain
	 */
	public void setGain(float fGain) {
		...
		// Better type
		if (line != null && line.isControlSupported(
	...
	public void run() {
		...
		if (audioFormat.getChannels() == 1) {
			...
			if (outputMode == 0) {
				...
				assert outputMode == 3 : "Unexpected output mode: " + 
				outputMode;
			}
		}
