...
/**
 * A class for using Google's Duplex Speech API. 
 * Allows for continuous recognition. Requires an API-Key. 
 * A duplex API opens two connections. One to an
 * upstream and one to a downstream. 
 * The system allows for continuous chunking on both up and downstream. 
 * This, in turn, allows for Google to return
...
public class GSpeechDuplex {
	 ...
	 */
	private static final String GOOGLE_DUPLEX_SPEECH_BASE = 
	...
	private List<GSpeechResponseListener> responseListeners = 
				new ArrayList<GSpeechResponseListener>();
	 ...
	 * 
	 * @param API_KEY
	 *    The API-Key for Google's Speech API. 
	...
	public void recognize(byte[] data , int sampleRate) {
		...
		final String API_DOWN_URL = GOOGLE_DUPLEX_SPEECH_BASE + 
								"down?maxresults=1&pair=" + PAIR;
		
		//Generates the Upstream URL
		final String API_UP_URL = GOOGLE_DUPLEX_SPEECH_BASE + "up?lang=" + 
	...
	public void recognize(TargetDataLine tl , AudioFormat af) throws 
		...
		final String API_UP_URL = GOOGLE_DUPLEX_SPEECH_BASE + "up?lang=" + 
							language + "&lm=dictation&client=chromium&pair=" + 
							PAIR + "&key=" + API_KEY + 
							//Tells Google to constantly monitor the stream;
	...
	/**
	 * This code opens a new Thread that connects to the downstream URL. 
	 * Due to threading, the best way to handle this is through the use of
	 * listeners.
	 * 
	 * @param The
	...
	private Thread downChannel(String urlStr) {
		...
		Thread downChannelThread = new Thread("Downstream Thread") {
			public void run() {
				...
				// of the audio at the applicable Bitrate. 
				// BR=sampleRate * bits per sample
				// Note that the TLS session uses "* SSLv3, TLS alert, 
				// Client hello (1): "to wake up the listener when there 
				// are additional bytes.
				// The mechanics of the TLS session should be transparent. 
				// Just use httpsUrlConn and allow it enough time to do its work
				...
				while (
					...
					File outputAudioFile = new File("oswelAudio.mp3");
	 ...
	 * @param mtl
	 *    The TargetDataLine you want to post data from. 
	 * 	  <b>Note should be open</b>
	 */
	private void openHttpsPostConnection(
		String murl, 
		TargetDataLine mtl, 
		int sampleRate) {
		...
		try {
			url = new URL(murl);
			HttpsURLConnection httpConn = getHttpsURLConnection(
				sampleRate, url);

			// this opens a connection, then sends POST & headers.
			final OutputStream out = httpConn.getOutputStream();
			//Note : if the audio is more than 15 seconds
			// don't write it to UrlConnInputStream all in one block as 
			// this sample does. Rather, segment the byteArray and on 
			...
			//Diagonostic Code.
			/*
			 * int resCode = httpConn.getResponseCode(); if (resCode / 100 != 2) 
			 * { System.out.println("ERROR"); } Scanner scanner = new
			 * Scanner(httpConn.getInputStream()); 
	...
	/**
	 * Force HttpsPostConnection Thread to Stop. 
	 * Closes this audio input stream and releases 
	...
	public void stopSpeechRecognition() {
		...
		if (ais != null)
			try {
				...
				// so forcing AudioSystem.write(....) to exit, 
				// so UpStream Thread will be closed
				ais.close();
			} catch (Exception ex) { 
				//catch a general exception here to have our mind peaceful
				ex.printStackTrace();
			}
	}
	
	/**
	 * Opens a chunked HTTPS post connection and 
	 * returns a Scanner with incoming data from Google Server 
	 * Used for to get UPStream Chunked HTTPS
	 ...
	 * @return A Scanner to access the server response. 
	 * 		(Probably will never be used)
	 */
	private Scanner openHttpsPostConnection(
		String urlStr, byte[][] data, int sampleRate) {
		...
		try {
			...
			// Note : if the audio is more than 15 seconds
			// dont write it to UrlConnInputStream all in one block 
			// as this sample does. Rather, segment the byteArray 
			// and on intermittently, sleeping thread supply bytes 
			// to the urlConn Stream at a rate that approaches
			// the bitrate ( =30K per sec. in this instance ).
			System.out.println("Starting to write");
			for (byte[] dataArray : mextrad) {
				out.write(dataArray); 
				// one big block supplied instantly to the 
				// underlying chunker wont work for duration > 15s.
				try {
					Thread.sleep(1000);
	 ...
	 */
	private HttpsURLConnection getHttpsURLConnection(int sampleRate , URL url) 
		throws IOException {
	...
	private byte[][] chunkAudio(byte[] data) {
		if (data.length >= MAX_SIZE) {//If larger than 1MB
			...
			for (int i = 0, j = 0; 
				i < data.length && j < data2D.length; i += frame, j++) {
				int length = 
				( data.length - i < frame ) ? data.length - i : frame;
			...
			System.arraycopy(
				data, 0, tmpData[0], 0, data.length);
			return tmpData;
		}
	}
}