...
import project.oswel.speechrecognition.utilities.Complex;
import project.oswel.speechrecognition.utilities.FFT;
...
public class MicrophoneAnalyzer extends Microphone {
	...
	/**
	 * Gets the volume of the microphone input Interval is 100ms so allow 
	 * 100ms for this method to run in your code or specify smaller interval.
	 * 
	 * @return The volume of the microphone input or -1 if data-line is 
	 *         not available
	 */
	public int getAudioVolume() { return getAudioVolume(100); }
	
	/**
	 * Gets the volume of the microphone input
	 * 
	 * @param interval:
	 *            The length of time you would like to calculate 
	 * 			  the volume over in milliseconds.
	 * @return The volume of the microphone input or -1 if data-line is 
	 * 		   not available.
	 */
	 ...
	 * @return The volume over the specified number of bytes or -1 
	 * 		   if data-line is unavailable.
	 */
	...
	/**
	 * Calculates the volume of AudioData which may be buffered 
	 * data from a data-line.
	 * 
	 * @param audioData
	...
	/**
	 * Returns the number of bytes over interval for useful when figuring 
	 * out how long to record.
	 * 
	 * @param seconds
	 ...
	 * @param seconds
	 *            The length in seconds
	 * @return the number of bytes the microphone will output over the 
	 * 		   specified time.
	 */
	public int getNumOfBytes(double seconds) {
		return (int) ( 
			seconds * getAudioFormat().getSampleRate() * 
	...
	/**
	 * Calculates the fundamental frequency. In other words, it 
	 * calculates pitch, except pitch is far more subjective and subtle. 
	 * Also note, that readings may occasionally, be in error due to the 
	 * complex nature of sound. This feature is in Beta
	 * 
	...
	public int getFrequency(int numOfBytes) {
		...
		//One byte is lost during conversion
	...
	/**
	 * Applies a Hanning Window to the data set. Hanning Windows are used 
	 * to increase the accuracy of the FFT. One should always apply a window 
	 * to a dataset before applying an FFT
	 * 
	 * @param The
	 *            data you want to apply the window to
	 * @param The
	 ...
	 * @return The windowed data set
	 */
	private double[] applyHanningWindow(
		double[] signalIn , int pos , int size) {
		for (int i = pos; i < pos + size; i++) {
			int j = i - pos; // j = index into Hann window function
			signalIn[i] = ( signalIn[i] * 0.5 * ( 1.0 - Math.cos(
				2.0 * Math.PI * j / size) ) 
			);
		}
		return signalIn;
	...
	/**
	 * This method calculates the fundamental frequency using Harmonic 
	 * Product Specturm It down samples the FFTData four times and multiplies 
	 * the arrays together to determine the fundamental frequency. 
	 * This is slightly more computationally expensive, but much more accurate. 
	 * In simpler terms, the function will remove the harmonic frequencies 
	 * which occur at every N value by finding the lowest common divisor 
	 * among them.
	 * 
	 * @param fftData
	 *            The array returned by the FFT
	 * @param N
	 *            the number of times you wish to downsample. 
	 * 			  WARNING: The more times you downsample, 
	 * 			  the lower the maximum detectable frequency is.
	...
	/**
	 * Removes useless data from transform since sound doesn't use 
	 * complex numbers.
	 * 
	 * @param The
	...
	/**
	 * Calculates the FFTbin size based off the length of the the 
	 * array Each FFTBin size represents the range of frequencies 
	 * treated as one. For example, if the bin size is 5 then the 
	 * algorithm is precise to within 5hz. Precondition: length 
	 * cannot be 0.
	 * 
	 * @param fftDataLength
	...
	/**
	 * Converts bytes from a TargetDataLine into a double[] allowing 
	 * the information to be read. NOTE: One byte is lost in the 
	 * conversion so don't expect the arrays to be the same length!
	 * 
	 * @param bufferData
	...
	private double[] bytesToDoubleArray(byte[] bufferData) {
		...
		for (int index = 0, floatIndex = 0; 
			index < bytesRecorded - bytesPerSample + 1; 
